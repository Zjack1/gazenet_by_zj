...
label_outpath = "../MPIIGaze/new_data/test_p14.txt"
# label_outpath = "../MPIIGaze/new_data/right_img_no_p13_label.txt"
...
def save_one_person(person_id: str, data_dir: pathlib.Path, eval_dir: pathlib.Path):
    ...
    for _, row in df.iterrows():
        ...
        if row.side == 'left':  # 非镜像，图像的左边就是真实的右边
            ...
            # 命名是按照镜像来的
            img_path = "../MPIIGaze/new_data/test_p14/" + str(person_id) + "_" + str(day) + "_" + str(index) + "_right.jpg"
            save_img_path = "MPIIGaze/new_data/test_p14/" + str(person_id) + "_" + str(day) + "_" + str(index) + "_right.jpg"
            cv2.imwrite(img_path, image)
            # if img_path in paths:  # 测试bug
            #     print(img_path)
...
def main():
    ...
    #for person_id in tqdm.tqdm(range(15)):  # 少了2个p13和p14
    for person_id in tqdm.tqdm([14]):  # 少了2个p13和p14
        ...
        #eval_dir = dataset_dir / 'Evaluation Subset' / 'sample list for eye image'  # 少了2个p13和p14
